  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--bg-base)] px-6 text-[var(--text-primary)]">
      <div className="panel w-full max-w-md p-8">
        <h1 className="font-[family-name:var(--font-display)] text-2xl text-[var(--text-primary)]">Sign in</h1>
        <p className="mt-2 text-sm text-[var(--text-muted)]">Use your workspace credentials to continue.</p>

        <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate>
          <div>
            <label htmlFor="email" className="mb-2 block text-sm text-[var(--text-primary)]">
              Email
            </label>
            <input
              id="email"
          ...
          <div>
            <label htmlFor="password" className="mb-2 block text-sm text-[var(--text-primary)]">
              Password
            </label>
            <div className="relative">
              <input id="password" type={showPassword ? "text" : "password"} className="focus-visible-ring w-full rounded-xl border border-[var(--border-hairline)] bg-[var(--bg-base)] px-3 py-2 pr-11 text-sm text-[var(--text-primary)]" value={password} onChange={(event) => setPassword(event.target.value)} />
